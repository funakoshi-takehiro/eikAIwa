...
EIK.Views.Categories = function (ctx) {
  ...
  EIK.Data.loadAll().then(function () {
    var cats = EIK.Data.allCategories();
    var rows = cats.map(function (c) {
      var list = EIK.Data.inCategory(c.id);
      var s = EIK.SRS.stats(list);
      var pct = s.total ? s.learned / s.total : 0;
      return '<a class="card catcard" href="#/categories/' + encodeURIComponent(c.id) + '">' +
...
EIK.Views.Category = function (ctx) {
  ...
  EIK.Data.loadAll().then(function () {
    ...
    var list = EIK.Data.inCategory(id);
    if (!c) {
    ...
    app.innerHTML = '<div class="stack-lg fade-in">' +
      ...
      (list.length
        ...
        : '<div class="card empty">このカテゴリの問題はまだ準備中です。</div>') +
      '</div>' +
