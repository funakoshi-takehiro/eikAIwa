...
| 7.5 | [記録を持たない](#75-学習の記録を持たない) | ログインが無い＝維持できない |
| 8 | [検証](#8-検証) | 3本のコマンド |
...
- **ダッシュを使わない**（`-` `―` `〜` すべて）。日本語の文中では翻訳調に見える。
  区切りたいときは全角の空き（`\u3000`）か句点で足りる。
- **飾りの英語ラベルを置かない**。見出しの上に `Speaking practice` や `Install` と
  英語を置く型は、読み飛ばされるうえに中身が増えない。
- **「AからBへ」の標語と3段の手順リストを書かない**。どちらも売り文句の型で、
  読まなくても分かった気になる。実際に何が起きるかをそのまま順に書く。

`browser_smoke.js` が、ホーム画面の本文にダッシュと大文字が現れていないことを
機械で見ている（アプリバーのロゴは商品名なので対象外）。
...
`localStorage` に置いてよいのは**表示と練習の設定だけ**。

理由は「あったら便利」ではなく「維持できない」から。ログイン機能が無いので
置き場所は `localStorage` しかなく、それは「その端末のそのブラウザ」の引き出しでしかない。

- スマホと PC で別物。同じ端末でも Safari と Chrome で別物
- プライベートウィンドウは閉じたら消える／「閲覧データを削除」で消える
- iOS はしばらく開かないと Safari が勝手に捨てることがある

この上に「連続日数 7 日」と表示すると、端末を替えた利用者に嘘をつくことになる。
実際、以前は Leitner box の復習アルゴリズムと進捗リングを載せていたが、
支えているものが上記だけだったので全部外した。**出題は毎回シャッフルする。**

戻したくなったら、まずログインとサーバー側の保存を用意すること。順序が逆にならないように。

`browser_smoke.js` が、練習を一周しても `localStorage` に何も書かれないこと、
設定を変えた後で書かれるのが `eikAIwa.v2` の1件だけであること、
その中身に `progress` / `streak` / `bookmark` などが混ざっていないことを機械で見ている。
