...
その他の機能: 読み上げ、ライト / ダーク、文字サイズ・行間の変更。

## 記録を残しません

**学習履歴・進捗・連続日数のたぐいは一切持ちません。**出題は毎回ランダムです。

ログイン機能が無いので、記録を置ける場所は `localStorage`
（その端末のそのブラウザの引き出し）しかありません。そこに置いた記録は、
...
という代物で、この上に「連続日数」や「復習の期限」を載せると、
端末を変えた瞬間にゼロに戻ります。維持できないものを維持しているように
見せることになるため、持たないことにしました。

保存しているのは**表示と練習の設定だけ**（テーマ・文字サイズ・読み上げなど）で、
どこにも送信しません。解析ツールも入れていません。アカウントもありません。
